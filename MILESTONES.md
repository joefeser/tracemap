...
- C# semantic `SymbolRelationship` facts for direct inheritance, interface implementation/extension, member overrides, and interface member implementations.
...
- SQLite `symbols`, `symbol_occurrences`, `fact_symbols`, and populated `symbol_relationships` tables for exact symbol queries without replacing facts.
...
- A semantic repo emits Tier1 `SymbolRelationship` facts with source/target symbol IDs for inheritance, interface implementation, and override relationships.
...
- `index.sqlite` contains queryable `symbols`, `symbol_occurrences`, `fact_symbols`, and `symbol_relationships`.
