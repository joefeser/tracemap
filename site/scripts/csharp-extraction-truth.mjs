...
async function validateIndexes(dist, errors) {
  ...
  if (!(await fileExists(blogPath)) || !hasHref(await readFile(blogPath, "utf8"), csharpExtractionTruthRoute)) errors.push(withEvidence("C# extraction truth blog index link is missing.", "blog/index.html"));
...
async function validatePage(pagePath, baseUrl, errors) {
  ...
  for (const pattern of forbiddenClaims) if (surfaces.some((surface) => pattern.test(surface))) errors.push(withEvidence(`C# extraction truth article contains unsupported positive claim: ${pattern}`, artifact));
  for (const pattern of rawPatterns) if (surfaces.some((surface) => pattern.test(surface))) errors.push(withEvidence(`C# extraction truth article contains source or executable material: ${pattern}`, artifact));
  for (const pattern of privatePatterns) if (surfaces.some((surface) => pattern.test(surface))) errors.push(withEvidence(`C# extraction truth article contains hard private material: ${pattern}`, artifact));
}
