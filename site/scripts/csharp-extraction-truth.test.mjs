...
test("C# extraction truth validator scans the public blog card", async (t) => {
  const root = await fixture(t); const metadataPath = join(root, "src/_blog/articles.json"); const articles = JSON.parse(await readFile(metadataPath, "utf8")); articles.find((article) => article.slug === "csharp-extraction-without-plausible-wrong-graphs").cardDescription = "TraceMap proves runtime reachability."; await writeFile(metadataPath, `${JSON.stringify(articles, null, 2)}\n`, "utf8"); await buildSite({ root, log() {} }); const errors = []; await validateCsharpExtractionTruthDist({ dist: join(root, "dist"), errors }); assert.match(errors.join("\n"), /blog card contains unsupported positive claim/);
});

test("C# extraction truth validator scans discovery text", async (t) => {
  for (const planted of ["The graph is complete.", "/Us" + "ers/example/workspace"]) await t.test(planted, async (subtest) => { const root = await fixture(subtest); await buildSite({ root, log() {} }); const discoveryPath = join(root, "dist/routes-index.json"); const discovery = JSON.parse(await readFile(discoveryPath, "utf8")); discovery.entries.find((entry) => entry.path === "/blog/csharp-extraction-without-plausible-wrong-graphs/").summary = planted; await writeFile(discoveryPath, `${JSON.stringify(discovery, null, 2)}\n`, "utf8"); const errors = []; await validateCsharpExtractionTruthDist({ dist: join(root, "dist"), errors }); assert.match(errors.join("\n"), /discovery entry contains unsupported positive claim|discovery entry contains hard private material/); });
});

test("C# extraction truth validator handles null discovery entries", async (t) => {
  const root = await fixture(t); await buildSite({ root, log() {} }); const discoveryPath = join(root, "dist/routes-index.json"); const discovery = JSON.parse(await readFile(discoveryPath, "utf8")); discovery.entries.unshift(null); await writeFile(discoveryPath, `${JSON.stringify(discovery, null, 2)}\n`, "utf8"); const errors = []; await validateCsharpExtractionTruthDist({ dist: join(root, "dist"), errors }); assert.deepEqual(errors, []);
});
