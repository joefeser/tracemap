...
test("validateDist accepts generated public sitemap and internal links", async () => {
  ...
  await validateDist({ requireAccessSafeEvidenceAcquisition: false, requireMsbuildBinlogEvidence: false, root });
});
...
test("validateDist normalizes trailing slash baseUrl values", async () => {
  ...
  await validateDist({ baseUrl: "https://tracemap.tools/", requireAccessSafeEvidenceAcquisition: false, requireMsbuildBinlogEvidence: false, root });
});
