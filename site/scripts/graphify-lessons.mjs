...
const forbiddenClaims = [/\bTraceMap\b[^.]{0,160}\b(?:proves?|guarantees?|certifies?|establishes?)\b[^.]{0,160}\b(?:runtime|complete|correct|safe|production|superior|parity)\b/i, /\b(?:TraceMap|Graphify)\b[^.]{0,100}\b(?:provides?|delivers?|offers?|achieves?|has|maintains?|ensures?|is|are)\s+(?:an?\s+)?(?:complete|correct|runtime-confirmed|production-ready|safe|superior|feature-parity)\b/i, /\bTraceMap (?:copied|ported|forked) Graphify\b/i, /\b(?:Graphify|TraceMap) is (?:better|superior|complete|production-ready|safe)\b/i, /\b(?:release approved|runtime reachability confirmed|production correctness established|safe to merge|safe to run)\b/i];
...
async function validateIndexes(dist, errors) {
  ...
  if (!entry) errors.push(withEvidence("Graphify lessons discovery entry is missing.", "routes-index.json"));
    ...
    const nonClaims = Array.isArray(entry.nonClaims) ? entry.nonClaims : [];
    scanSafety([entry.title, entry.summary, ...limitations, ...nonClaims].filter((value) => typeof value === "string"), "Graphify lessons discovery entry", "routes-index.json", errors);
  }
}

async function validatePage(pagePath, baseUrl, errors) {
  const html = await readFile(pagePath, "utf8"); const browserDecoded = decodeBrowserNumericEntities(html); const decoded = decodeHtmlEntities(browserDecoded); const rendered = normalizeRenderedText(browserDecoded); const collapsed = decodeHtmlEntities(stripTagsQuoteAware(decoded)).replace(/\s+/g, " ").trim();
