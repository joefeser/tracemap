...
const requiredText = ["Public claim level: concept", "fail-closed invariant", "synthetic fixture", "versioned rule", "rule ID", "evidence tier", "Tier 1", "Tier 2", "Tier 3", "Tier 4", "commit SHA", "extractor ID/version", "AnalysisGap", "coverage reduced", "persistence", "readback", "downstream propagation", "documented limitations", "owner question"];
...
async function validatePage(pagePath, baseUrl, errors) {
  ...
  if (!new RegExp(`<link\\b[^>]*rel=["']canonical["'][^>]*href=["']${escapeRegExp(baseUrl)}${escapeRegExp(gapBecomesRuleRoute)}["']`, "i").test(html)) errors.push(withEvidence("Gap-to-rule canonical URL is missing.", artifact));
