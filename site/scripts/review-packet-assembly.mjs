...
export const reviewPacketAssemblyRoute = "/packets/assembly/";
export const reviewPacketAssemblyRequiredLinks = [
  "/packets/",
  "/manager-packet/",
...
export const reviewPacketAssemblyInboundRoutes = ["/packets/", "/review-room/"];

const pageArtifact = "packets/assembly/index.html";
...
const requiredText = [
  ...
  "This is not a generated packet-builder feature",
  "A public-safe trail or named private review location",
  "Missing fields stay visible as limitations"
];
...
const metadataNonClaimTerms = [
  ...
  "outage cause",
  "release approval or safety",
  "operational safety",
  "AI impact analysis",
  "LLM analysis",
  "autonomous review",
  "generated packet-builder behavior",
  "complete coverage"
];
...
const forbiddenClaimPatterns = [
  ...
  /\bgenerated packet-builder (?:feature|output|workflow)\b/i
];

const sanctionedBoundarySectionPattern =
  /<section\b(?=[^>]*\bdata-boundary-region\b)[^>]*>[\s\S]*?<\/section>/gi;

export async function validateReviewPacketAssemblyDist({ baseUrl = "https://tracemap.tools", dist, errors }) {
  const localErrors = [];
  const cleanBaseUrl = normalizeBaseUrl(baseUrl, localErrors);
  const pagePath = resolve(dist, "packets", "assembly", "index.html");

  if (!(await fileExists(pagePath))) {
    localErrors.push(withEvidence("Review packet assembly page is missing required public route: /packets/assembly/", pageArtifact));
  ...
  await validateReviewPacketAssemblyPage({ pagePath, routeContext, errors: localErrors });
...
function normalizeBaseUrl(value, errors) {
  ...
  try {
    ...
    errors.push(withEvidence(`Review packet assembly baseUrl must be a valid absolute URL: ${String(value)}`, "baseUrl input"));
    return null;
  }

  if (url.protocol !== "https:" && url.protocol !== "http:") {
    errors.push(withEvidence(`Review packet assembly baseUrl must use http or https: ${String(value)}`, "baseUrl input"));
    return null;
  }
...
async function validateSitemap({ baseUrl, dist, errors }) {
  ...
  if (!sitemapUrls.has(`${baseUrl}${reviewPacketAssemblyRoute}`)) {
    errors.push(withEvidence(`Review packet assembly sitemap is missing required route: ${baseUrl}${reviewPacketAssemblyRoute}`, sitemapArtifact));
  }
}
...
async function readRouteContext({ dist, errors }) {
  ...
  try {
    ...
    errors.push(withEvidence(`Review packet assembly could not parse routes-index.json: ${error.message}`, routesIndexArtifact));
    return { routeEntry, routes, sitemapRoutes };
  }

  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed) || !Array.isArray(parsed.entries)) {
    errors.push(withEvidence("Review packet assembly routes-index.json is invalid: expected entries array", routesIndexArtifact));
    return { routeEntry, routes, sitemapRoutes };
  }
...
function validateRouteEntry(routeEntry, errors) {
  if (!routeEntry) {
    errors.push(withEvidence(`Review packet assembly routes-index.json is missing required route: ${reviewPacketAssemblyRoute}`, routesIndexArtifact));
    return;
  }
  ...
  for (const [field, expected] of Object.entries(expectedFields)) {
    if (routeEntry[field] !== expected) {
      errors.push(withEvidence(`Review packet assembly routes-index.json expected ${field} ${expected}, got ${String(routeEntry[field])}`, routesIndexArtifact));
    }
  }

  if (!Array.isArray(routeEntry.limitations) || routeEntry.limitations.length === 0) {
    errors.push(withEvidence("Review packet assembly routes-index.json must include limitations metadata.", routesIndexArtifact));
  }

  if (!Array.isArray(routeEntry.nonClaims) || routeEntry.nonClaims.length === 0) {
    errors.push(withEvidence("Review packet assembly routes-index.json must include nonClaims metadata.", routesIndexArtifact));
    return;
  }
...
async function validateReviewPacketAssemblyPage({ pagePath, routeContext, errors }) {
  ...
  for (const phrase of requiredText) {
    if (!pageText.includes(phrase)) {
      errors.push(withEvidence(`Review packet assembly page is missing required text: ${phrase}`, pageArtifact));
    }
  }

  for (const ingredient of requiredIngredients) {
    if (!lowerPageText.includes(ingredient.toLowerCase())) {
      errors.push(withEvidence(`Review packet assembly page is missing required ingredient: ${ingredient}`, pageArtifact));
    }
    if (!hasIngredientRow(html, ingredient)) {
      errors.push(withEvidence(`Review packet assembly page is missing ingredient row: ${ingredient}`, pageArtifact));
    }
  }

  for (const section of requiredWorkflowSections) {
    if (!pageText.includes(section)) {
      errors.push(withEvidence(`Review packet assembly page is missing workflow section: ${section}`, pageArtifact));
    }
  }

  for (const condition of requiredStopConditions) {
    if (!lowerPageText.includes(condition)) {
      errors.push(withEvidence(`Review packet assembly page is missing stop condition: ${condition}`, pageArtifact));
    }
  }

  for (const tier of requiredEvidenceTiers) {
    if (!pageText.includes(tier)) {
      errors.push(withEvidence(`Review packet assembly page is missing evidence tier: ${tier}`, pageArtifact));
    }
  }

  for (const link of reviewPacketAssemblyRequiredLinks) {
    if (!hasHref(html, link)) {
      errors.push(withEvidence(`Review packet assembly page is missing required link: ${link}`, pageArtifact));
    }
  }
  ...
  if (wordCount < 400 || wordCount > 1500) {
    errors.push(withEvidence(`Review packet assembly page word count must be between 400 and 1500 words, got ${wordCount}`, pageArtifact));
  }
...
function validatePageMetadata(html, errors) {
  ...
  const checks = [
    ...
    [
      linkTags.some(
        (attributes) =>
          ...
          getAttribute(attributes, "href") === "https://tracemap.tools/packets/assembly/"
      ),
      "canonical URL"
    ],
    ...
    [
      hasMeta(metaTags, {
        property: "og:url",
        content: "https://tracemap.tools/packets/assembly/"
      }),
      "Open Graph URL"
    ]
  ];
  ...
  for (const [passed, label] of checks) {
    if (!passed) {
      errors.push(withEvidence(`Review packet assembly page is missing required metadata: ${label}`, pageArtifact));
    }
  }
}
...
function validateInternalRouteLinks(html, { routes, sitemapRoutes }, errors) {
  ...
  for (const href of extractHrefs(extractMainHtml(html))) {
    ...
    if (!routes.has(route) && !sitemapRoutes.has(route)) {
      errors.push(withEvidence(`Review packet assembly page links to unresolved internal route: ${route}`, pageArtifact));
    }
  }
}
...
async function validateInboundLinks({ dist, errors }) {
  ...
  for (const route of reviewPacketAssemblyInboundRoutes) {
    ...
    const html = await readFile(pagePath, "utf8");
    if (!hasHref(html, reviewPacketAssemblyRoute)) {
      missing.push(route);
    }
  }

  if (missing.length > 0) {
    errors.push(withEvidence(`Review packet assembly is missing inbound links from live adjacent routes: ${missing.join(", ")}`, "adjacent route HTML"));
  }
}
  ...
  for (const pattern of forbiddenClaimPatterns) {
    ...
    for (const match of text.matchAll(globalPattern)) {
      if (!hasNegatedContext(text, match.index)) {
        errors.push(withEvidence(`Review packet assembly contains forbidden public claim in ${label}: ${match[0]}`, pageArtifact));
        break;
      }
    }
  }
...
function validateRawMaterial({ errors, text, label }) {
  for (const pattern of rawMaterialPatterns) {
    if (pattern.test(text)) {
      errors.push(withEvidence(`Review packet assembly contains forbidden raw/private material ${label}: ${pattern.source}`, pageArtifact));
    }
  }
}
...
function validateHardPrivateMaterial({ errors, text, label }) {
  for (const pattern of hardPrivatePatterns) {
    if (pattern.test(text)) {
      errors.push(withEvidence(`Review packet assembly contains hard private material in ${label}: ${pattern.source}`, pageArtifact));
    }
  }
}

function hasNegatedContext(value, index) {
  const prefix = value.slice(Math.max(0, index - 48), index).toLowerCase();
  return /(?:not|no|without|cannot|can't|does not|do not|never)\s+(?:a\s+)?(?:new\s+)?$/.test(prefix);
}

function hasIngredientRow(html, ingredient) {
  const escaped = escapeRegExp(ingredient);
  return new RegExp(`<tr\\b(?=[^>]*\\bdata-packet-ingredient\\s*=\\s*["']${escaped}["'])`, "i").test(html);
}
