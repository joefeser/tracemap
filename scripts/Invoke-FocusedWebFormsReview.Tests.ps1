...
Assert-True ($content.Contains('if (-not $Projectless -and [string]::IsNullOrWhiteSpace($SolutionRelativePath) -and $ProjectRelativePath.Count -eq 0)', [StringComparison]::Ordinal)) "project-only and projectless invocation must not prompt for a solution"
...
Assert-True ($content.Contains('if (-not $Projectless -and $ProjectRelativePath.Count -eq 0 -and [string]::IsNullOrWhiteSpace($SolutionRelativePath))', [StringComparison]::Ordinal)) "solution-only and projectless invocation must not prompt for project paths"
