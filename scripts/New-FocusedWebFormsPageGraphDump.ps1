...
if ($scanDirectory -notmatch '^[A-Za-z0-9._-]+$' -or $scanDirectory -in @('.', '..')) { throw 'WEBFORMS_PAGE_GRAPH_DUMP_INDEX_LAYOUT_INVALID' }
if ($scanDirectory -eq 'combined') { throw 'WEBFORMS_PAGE_GRAPH_DUMP_COMBINED_INDEX_UNSUPPORTED_USE_TRAVERSAL_SUMMARY' }
$indexPath = [IO.Path]::GetFullPath((Join-Path $root "$scanDirectory/index.sqlite"))
$reviewRootPrefix = $root + [IO.Path]::DirectorySeparatorChar
