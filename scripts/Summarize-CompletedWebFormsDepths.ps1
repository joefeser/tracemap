    ...
    try {
        ...
        foreach ($gap in $root.GetProperty('gaps').EnumerateArray()) {
            ...
            if ($classification -eq 'TruncatedByLimit') {
                ...
                if ($reason -cnotin @('cycle','depth','frontier','path')) { $reason = 'unavailable' }
                $counts[$reason]++
            }
        ...
        return @{
            ...
            MissingHandler=$missingHandler; MissingTerminal=$missingTerminal; Gaps=$counts
        }
    }
    finally {
...
foreach ($entry in @(@{Depth=8; Data=$a}, @{Depth=10; Data=$b})) {
    ...
    Write-Host "depth=$d|cycle=$($s.Gaps.cycle)|depthGap=$($s.Gaps.depth)|frontier=$($s.Gaps.frontier)|path=$($s.Gaps.path)|unknownReason=$($s.Gaps.unavailable)|otherLimits=$($s.Gaps.otherLimits)"
}
