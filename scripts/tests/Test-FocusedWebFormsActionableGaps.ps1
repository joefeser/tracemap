...
try {
    $chains = @(
        ...
        @{ surfaceId = 's3'; handlerFactId = 'private-handler-3'; terminalKind = $null; supportingFactIds = @('private-3'); ruleIds = @('legacy.webforms.event-flow.v1'); evidenceTiers = @('Tier2Structural'); coverageLabels = @('bounded-static-webforms-flow'); traversalObservation = @{ stopState = 'observed-downstream-without-supported-terminal' } },
