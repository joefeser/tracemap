...
$packet = [ordered]@{
    ...
    summary = @{ projectCount = 1; surfaceCount = 2; eventChainCount = 2; downstreamBoundaryCount = 1; identityStateCount = 1; batchDataMovementCount = 1; structuralSliceCandidateCount = 1; clientBehaviorCount = 2; serverBehaviorCount = 2; gapCount = 1; truncated = $false }
...
try {
    ...
    foreach ($expected in @('Go.Click', 'App.First.Go_Click()', 'Pages/First.aspx:L2-2', 'Pages/First.aspx.cs:L10-14', 'SaveAudit.ProcessRequest/1', 'api/SaveAudit.ashx.vb:L3-8', 'Inline client behavior (2)', 'generated-client-id', 'Pages/First.aspx:L20-24', 'POST ashx', 'SaveAudit.ashx', 'api/SaveAudit.ashx', 'unique-repository-handler-file', 'Server behavior (2)', 'Go.enabled', 'Pages/First.aspx.vb:L30-30', 'Sample.Controls.OrderPolicy', 'App_Code/Controls/OrderPolicy.vb', 'Evidence-backed behavior summary', '1 of 1 client event binding(s) correlate to one retained server handler', '1 of 1 inline HTTP request(s) join through a handler declaration to one retained entry method', 'Boundary status:</strong> 1 detected', 'stored-procedure-candidate', 'HandlerTerminalUnavailable', 'Tier4Unknown', ('a' * 40), 'legacy-webforms/1', 'identity-one', 'candidate-one', 'Raw source omitted')) {
    ...
    if ($handoff.subject.filePath -ne 'Pages/First.aspx' -or $handoff.counts.eventChains -ne 2 -or $handoff.counts.clientBehaviors -ne 2 -or $handoff.counts.serverBehaviors -ne 2 -or $handoff.analysis.boundaryStatus -ne '1 detected' -or $handoff.evidenceDocs.status -ne 'supplied-read-only') { throw 'Page handoff projection was incomplete.' }
