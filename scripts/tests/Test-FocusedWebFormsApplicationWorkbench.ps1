...
$packet = [ordered]@{
    ...
    summary = @{ projectCount = 1; surfaceCount = 2; eventChainCount = 5; downstreamBoundaryCount = 1; identityStateCount = 1; batchDataMovementCount = 1; structuralSliceCandidateCount = 1; clientBehaviorCount = 3; serverBehaviorCount = 2; gapCount = 1; truncated = $false }
...
try {
    ...
    foreach ($heading in @('<th>Calls projected / unique</th>','<th>Handler unavailable</th>','<th>Downstream / no terminal</th>','<th>No downstream</th>','<th>Truncated</th>','<th>Other incomplete</th>','<th>Evidence gaps</th>')) {
    ...
    if ($handoff.counts.retainedCalls -ne 2 -or $handoff.counts.chainAssociatedRetainedCalls -ne 2 -or $handoff.counts.uniqueRetainedCallFacts -ne 1 -or $handoff.chainOutcomes.unresolvedHandlers -ne 1 -or $handoff.chainOutcomes.downstreamWithoutSupportedTerminal -ne 2 -or $handoff.chainOutcomes.otherIncomplete -ne 1) { throw 'Page handoff omitted chain-outcome or retained-call counts.' }
