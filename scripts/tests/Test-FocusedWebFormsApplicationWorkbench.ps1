...
$packet = [ordered]@{
    ...
    summary = @{ projectCount = 1; surfaceCount = 2; eventChainCount = 1; downstreamBoundaryCount = 1; identityStateCount = 1; batchDataMovementCount = 1; structuralSliceCandidateCount = 1; clientBehaviorCount = 1; serverBehaviorCount = 1; gapCount = 1; truncated = $false }
...
try {
    ...
    foreach ($expected in @('Go.Click', 'App.First.Go_Click()', 'Pages/First.aspx:L2-2', 'Pages/First.aspx.cs:L10-14', 'Inline client behavior (1)', 'generated-client-id', 'Pages/First.aspx:L20-24', 'Server behavior (1)', 'Go.enabled', 'Pages/First.aspx.vb:L30-30', 'stored-procedure-candidate', 'HandlerTerminalUnavailable', 'Tier4Unknown', ('a' * 40), 'legacy-webforms/1', 'identity-one', 'candidate-one', 'Raw source omitted')) {
    ...
    if ($handoff.subject.filePath -ne 'Pages/First.aspx' -or $handoff.counts.eventChains -ne 1 -or $handoff.counts.clientBehaviors -ne 1 -or $handoff.counts.serverBehaviors -ne 1 -or $handoff.evidenceDocs.status -ne 'supplied-read-only') { throw 'Page handoff projection was incomplete.' }
    ...
    if ($handoff.inventories.serverBehavior[0].id -ne 'server-one' -or $handoff.inventories.serverBehavior[0].evidenceFactId -ne 'fact-server-one') { throw 'Page handoff omitted server behavior evidence.' }
