...
$packet = [ordered]@{
    ...
    summary = @{ projectCount = 1; surfaceCount = 2; eventChainCount = 4; downstreamBoundaryCount = 1; identityStateCount = 1; batchDataMovementCount = 1; structuralSliceCandidateCount = 1; clientBehaviorCount = 3; serverBehaviorCount = 2; gapCount = 1; truncated = $false }
...
try {
    ...
    foreach ($heading in @('<th>Calls projected / unique</th>','<th>Handler unavailable</th>','<th>Downstream / no terminal</th>','<th>No downstream</th>','<th>Truncated</th>','<th>Evidence gaps</th>')) {
