...
try {
    ...
    $receipt.stages.workbench.state = 'completed'; $receipt.stages.workbench.completedUtc = [DateTime]::UtcNow.ToString('O'); $receipt.stages.workbench.artifacts = New-ArtifactReceipt $root $workbenchArtifacts; Write-RunReceipt $receiptPath $receipt
