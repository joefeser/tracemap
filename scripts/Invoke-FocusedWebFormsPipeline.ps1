...
if (Test-Path -LiteralPath $receiptPath -PathType Leaf) {
    ...
    $selectionProperty = $receipt.PSObject.Properties['selection']
    $receiptRunModeProperty = if ($null -eq $selectionProperty -or $null -eq $selectionProperty.Value) {
        $null
    } else {
        $selectionProperty.Value.PSObject.Properties['runMode']
    }
...
try {
    ...
    elseif (!(Assert-CompletedStage $root $receipt.stages.scan 'scan')) {
        $activeStage = 'scan'
        $retryFailedScan = [string]$receipt.stages.scan.state -eq 'failed'
        Set-StageState $receipt $activeStage 'running'
        Write-RunReceipt $receiptPath $receipt
        if ($retryFailedScan) {
            foreach ($failedScanArtifact in @($scanPath, (Join-Path $root '.scan-stage'))) {
                if (Test-Path -LiteralPath $failedScanArtifact) { Remove-Item -LiteralPath $failedScanArtifact -Recurse -Force }
            }
        }
    ...
    if (!(Assert-CompletedStage $root $receipt.stages.workbench 'workbench')) {
        ...
        $workbenchArtifacts = @(
            ...
            Get-ChildItem -LiteralPath $workbenchDirectory -File -Filter '*.handoff.json' | ForEach-Object { $_.FullName }
        )
        Set-StageState $receipt 'workbench' 'completed' (New-ArtifactReceipt $root $workbenchArtifacts)
        Write-RunReceipt $receiptPath $receipt
