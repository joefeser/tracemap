param([string]$MethodName = '', [string]$IndexPath = '', [string]$ReportPath = '', [string]$OutputRoot = '', [string]$ConfigPath = '')
...
& (Join-Path $PSScriptRoot 'Test-FocusedWebFormsRawEvidence.ps1') -IndexPath $IndexPath -ReportPath $ReportPath -OutputRoot $OutputRoot -ConfigPath $ConfigPath -CreateLocalInspection -StartingMethodName $MethodName.Trim()
