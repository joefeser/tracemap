...
try {
    ...
    foreach ($surface in $ordered) {
        ...
        $batch = @(Values $packet.batchDataMovementInventory | Where-Object { $null -ne $_.projectId -and (Same $_.projectId $surface.projectId) } | Sort-Object batchDataMovementId)
        ...
        foreach ($item in @($boundaries + $identity + $batch + $candidates)) {
        ...
        $evidence = @($surface.evidence) + @(Values $surface.supportingEvidence) + @($chains | ForEach-Object { Values $_.evidence }) + @($boundaries | ForEach-Object { Values $_.evidence }) + @($identity | ForEach-Object { $_.evidence }) + @($batch | ForEach-Object { $_.evidence }) + @($candidates | ForEach-Object { Values $_.evidence })
        ...
        $handoff = [ordered]@{
            ...
            inventories = [ordered]@{
                ...
                projectDataMovement = @($batch | ForEach-Object { [ordered]@{ id = [string]$_.batchDataMovementId; surfaceKind = [string]$_.surfaceKind; mechanism = [string]$_.mechanism; operationKind = [string]$_.operationKind; ownerStatus = [string]$_.ownerStatus; projectResolution = [string]$_.projectResolution; safeMetadata = $_.safeMetadata; evidenceFactId = [string]$_.evidence.factId; supportingFactIds = @(Values $_.supportingFactIds) } })
        ...
        $batchRows = foreach ($item in $batch) { '<tr><td><code>{0}</code></td><td>{1}</td><td>{2}</td><td>{3}</td><td><code>{4}</code></td></tr>' -f (ConvertTo-HtmlText $item.batchDataMovementId), (ConvertTo-HtmlText $item.mechanism), (ConvertTo-HtmlText $item.operationKind), (ConvertTo-HtmlText $item.projectResolution), (ConvertTo-HtmlText $item.evidence.factId) }
...
<details><summary><strong>Project-scoped data movement ($($batch.Count))</strong></summary><p>These rows are associated through the page project and require human scoping to a specific page.</p><table><thead><tr><th>ID</th><th>Mechanism</th><th>Operation</th><th>Resolution</th><th>Evidence</th></tr></thead><tbody>$($batchRows -join '')</tbody></table></details>
