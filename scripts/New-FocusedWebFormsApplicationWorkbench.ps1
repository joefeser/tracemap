...
function Outlier-Table([string]$Title, [object[]]$Rows) {
    ...
    $body = foreach ($row in @($Rows | Select-Object -First 25)) {
        '<tr><td><code>{0}</code></td><td>{1}</td><td>{2}</td><td>{3} / {4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td><td>{9}</td><td>{10}</td></tr>' -f `
            (ConvertTo-HtmlText $row.pageId), $row.counts.controls, $row.counts.eventChains,
            $row.counts.callProjections, $row.counts.uniqueCallFacts, $row.counts.projectionReuse,
    ...
    '<details open><summary><strong>{0}</strong></summary><table><thead><tr><th>Page</th><th>Controls</th><th>Chains</th><th>Calls projected / unique</th><th>Reuse</th><th>Handler unavailable</th><th>Other incomplete</th><th>Truncated</th><th>Boundaries</th><th>Gaps</th></tr></thead><tbody>{1}</tbody></table></details>' -f (ConvertTo-HtmlText $Title), ($body -join '')
}
...
try {
    ...
    foreach ($surface in $ordered) {
        ...
        $truncatedChainCount = @($chains | Where-Object {
            $_.traversalObservation.stopState -like '*truncated*' -or
            (Property-Value $_.traversalObservation 'truncated') -eq $true
        }).Count
        ...
        $handoff = [ordered]@{
            ...
            eventChains = @($chains | ForEach-Object { [ordered]@{
                ...
                callEvidence = @((Values $_.callEvidence) | ForEach-Object { [ordered]@{ callEvidenceId = [string]$_.callEvidenceId; calleeName = [string]$_.calleeName; callKind = [string]$_.callKind; evidence = Project-Evidence $_.evidence; limitations = @(Values $_.limitations) } })
        ...
        $chainRows = foreach ($chain in $chains) {
            ...
            $displayedCalls = @($calls | Select-Object -First 24)
            $callItems = @($displayedCalls | ForEach-Object {
                '<li><code>{0}</code> <small>{1}; {2}:L{3}-{4}</small></li>' -f (ConvertTo-HtmlText $_.calleeName), (ConvertTo-HtmlText $_.callKind), (ConvertTo-HtmlText $_.evidence.filePath), (ConvertTo-HtmlText $_.evidence.startLine), (ConvertTo-HtmlText $_.evidence.endLine)
            })
            $callTotal = if ($null -ne $chain.callEvidenceTotalCount) { [int]$chain.callEvidenceTotalCount } else { $calls.Count }
            $callNote = if ($chain.callEvidenceTruncated -or $callTotal -gt $displayedCalls.Count) { "<small>$($displayedCalls.Count) shown of $callTotal retained; bounded output.</small>" } else { '' }
            $callHtml = if ($calls.Count -gt 0) { '<details class="calls"><summary>{0} retained</summary><ul>{1}</ul>{2}</details>' -f $callTotal, ($callItems -join ''), $callNote } else { '<span class="muted">none retained</span>' }
...
<section class="card"><h2>Trigger and retained call paths</h2><table><thead><tr><th>Chain</th><th>Event source</th><th>Handler</th><th>Retained calls</th><th>Classification</th><th>Terminal/stop</th></tr></thead><tbody>$($chainRows -join '')</tbody></table><p class="muted">Call names and spans are bounded static evidence. They do not prove receiver type, overload resolution, branch execution, or runtime dispatch.</p></section>
...
"@
        ...
        $pageRows.Add([pscustomobject]@{ PageId = $pageId; Path = [string]$surface.evidence.filePath; SurfaceKind = [string]$surface.surfaceKind; Controls = @(Values $surface.controlIds).Count; Chains = $chains.Count; ClientBehaviors = $clientBehavior.Count; ServerBehaviors = $serverBehavior.Count; Boundaries = $boundaries.Count; CallProjections = $chainAssociatedCallCount; UniqueCallFacts = $uniqueRetainedCallCount; HandlerUnavailable = $unresolvedHandlerCount; NoTerminal = $downstreamWithoutTerminalCount; NoDownstream = $noDownstreamCount; Truncated = $truncatedChainCount; OtherIncomplete = $otherIncompleteCount; Gaps = $gaps.Count; GapCategories = @($gapCategories); Coverage = ($coverage -join ', '); Verdict = [string]$decision.verdict; Disposition = [string]$decision.migrationDisposition })
    ...
    $rankedRows = @($pageRows | Sort-Object `
        ...
        @{ Expression = { [int]$_.UniqueCallFacts }; Descending = $true }, `
    ...
    foreach ($row in $rankedRows) {
        ...
        if ($row.Truncated -gt 0) { $signals.Add('truncated-chain') }
    ...
    $outlierArtifact = [ordered]@{
        ...
        ordering = @('truncated desc','handler-unavailable desc','other-incomplete desc','gaps desc','unique-call-facts desc','projection-reuse desc','page-id asc')
        pages = @($outlierPages)
        limitations = @(
    ...
    $outlierSections = @(
        ...
        Outlier-Table 'Highest unique retained call counts' @($outlierPages | Where-Object { $_.counts.uniqueCallFacts -gt 0 } | Sort-Object @{ Expression = { $_.counts.uniqueCallFacts }; Descending = $true }, pageId)
        ...
        Outlier-Table 'Retained calls without an observed boundary' @($outlierPages | Where-Object { $_.counts.uniqueCallFacts -gt 0 -and $_.counts.boundaries -eq 0 } | Sort-Object @{ Expression = { $_.counts.uniqueCallFacts }; Descending = $true }, pageId)
    )
    $outlierHtml = @"
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Alias-only Web Forms outlier review</title><style>:root{font-family:system-ui,sans-serif;color:#172033;background:#f5f7fb}main{max-width:1500px;margin:auto;padding:24px}.shareable{padding:12px;border-left:5px solid #287a36;background:#effaf1}table{width:100%;border-collapse:collapse;background:white}th,td{padding:9px;border:1px solid #dbe2ee;text-align:left;vertical-align:top}th{background:#eaf1ff}code{background:#edf1f7;padding:.1rem .3rem;border-radius:4px}details{background:white;border:1px solid #dbe2ee;border-radius:8px;padding:14px;margin:14px 0}</style></head><body><main><h1>Alias-only Web Forms outlier review</h1><p class="shareable">Anonymous count projection for $($outlierPages.Count) pages. It contains page aliases and retained counts only; paths, symbols, repository identifiers, packet identifiers, scan identifiers, and commit SHAs are omitted.</p><p>The inspection order is deterministic: truncated chains, unavailable handlers, other incomplete chains, gaps, unique calls, projection reuse, then page alias. It is not a business-priority or migration-effort score.</p>$($outlierSections -join '')<p>Static evidence does not prove runtime execution, business intent, migration effort, or absence under partial coverage.</p></main></body></html>
"@
