...
try {
    ...
    foreach ($surface in $ordered) {
        ...
        $clientBehaviorRows = foreach ($item in $clientBehavior) {
            $eventOrMutation = if ($item.safeMetadata.clientEventName -and $item.safeMetadata.clientEventName -ne 'not-applicable') { $item.safeMetadata.clientEventName } elseif ($item.safeMetadata.mutationKinds -and $item.safeMetadata.mutationKinds -ne 'not-applicable') { $item.safeMetadata.mutationKinds } elseif ($item.safeMetadata.constraintKind -and $item.safeMetadata.constraintKind -ne 'not-applicable') { "$($item.safeMetadata.constraintKind)=$($item.safeMetadata.constraintValue)" } else { 'not-applicable' }
            $control = if ($item.safeMetadata.controlId -and $item.safeMetadata.controlId -ne 'unresolved') { $item.safeMetadata.controlId } else { 'unresolved' }
            $serverHandler = if ($item.safeMetadata.serverHandlerName -and $item.safeMetadata.serverHandlerName -ne 'not-applicable') { $item.safeMetadata.serverHandlerName } else { 'not-applicable' }
