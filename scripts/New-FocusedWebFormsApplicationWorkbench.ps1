...
<details open><summary><strong>Server behavior ($($serverBehavior.Count))</strong></summary><table><thead><tr><th>Kind</th><th>Handler</th><th>Control / operation</th><th>Branch</th><th>Resolution / detail</th><th>Evidence span</th></tr></thead><tbody>$($serverBehaviorRows -join '')</tbody></table><p class="muted">Projectless VB syntax candidates only; branch execution, navigation, request termination, and rendered control state are not proven.</p></details>
...
"@
        ...
        $pageRows.Add([pscustomobject]@{ PageId = $pageId; Path = [string]$surface.evidence.filePath; SurfaceKind = [string]$surface.surfaceKind; Chains = $chains.Count; ClientBehaviors = $clientBehavior.Count; ServerBehaviors = $serverBehavior.Count; Boundaries = $boundaries.Count; CallProjections = $chainAssociatedCallCount; UniqueCallFacts = $uniqueRetainedCallCount; HandlerUnavailable = $unresolvedHandlerCount; NoTerminal = $downstreamWithoutTerminalCount; NoDownstream = $noDownstreamCount; Truncated = $truncatedChainCount; Gaps = $gaps.Count; Coverage = ($coverage -join ', '); Verdict = [string]$decision.verdict; Disposition = [string]$decision.migrationDisposition })
