...
<section class="card"><h2>Trigger and retained call paths</h2><table><thead><tr><th>Chain</th><th>Event source</th><th>Handler</th><th>Retained calls</th><th>Classification</th><th>Terminal/stop</th></tr></thead><tbody>$($chainRows -join '')</tbody></table><p class="muted">Normalized call sites collapse syntax and semantic facts only when their retained site identity matches. Compiler-resolved declaring types and technology families are static evidence; they do not prove runtime receiver type, branch execution, or dynamic dispatch.</p></section>
