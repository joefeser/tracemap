...
public sealed class ProjectlessVisualBasicWebFormsDiagnosticsTests
{
    ...
    public async Task Projectless_vb_webforms_handler_reaches_explicit_data_adapter_fill_boundary()
    {
        ...
        var chains = packet.EventChains
            .Where(item => item.HandlerSymbol == "DefaultPage.LoadButton_Click(Object,EventArgs)")
            .ToArray();
        ...
        var boundaries = packet.DownstreamBoundaries
            .Where(item => chains.Any(chain => chain.ChainId == item.ChainId))
            .ToArray();
    ...
    [Fact]
    public async Task Projectless_vb_handlers_retain_canonical_signature_syntax_calls()
    {
        ...
        var search = Assert.Single(packet.EventChains, chain => chain.HandlerSymbol == "ReviewBoard.SearchButton_Click(Object,EventArgs)");
        ...
        var approve = Assert.Single(packet.EventChains, chain => chain.HandlerSymbol == "ReviewBoard.ApproveButton_Click(Object,EventArgs)");
        ...
        var uiOnly = Assert.Single(packet.EventChains, chain => chain.HandlerSymbol == "ReviewBoard.ToggleButton_Click(Object,EventArgs)");
