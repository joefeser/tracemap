...
struct TraceMapSwiftSmokeTests {
    ...
    static func swiftStorageDataSurfaceFactsAreEmittedSafely() throws {
        ...
        assert(storageFacts.contains { $0.factType == "SwiftUserDefaultsKeyAccessed" && $0.properties["operationDirection"] == "read" && $0.properties["normalizedKey"] == "firstScopedPreference" })
        assert(storageFacts.contains { $0.factType == "SwiftUserDefaultsKeyAccessed" && $0.properties["operationDirection"] == "read" && $0.properties["normalizedKey"] == "secondScopedPreference" })
        assert(!storageFacts.contains { $0.factType == "SwiftUserDefaultsKeyAccessed" && $0.properties["normalizedKey"] == "staticScopedPreference" })
        assert(!storageFacts.contains { $0.factType == "SwiftUserDefaultsKeyAccessed" && ($0.properties["normalizedKey"]?.hasPrefix("user-") ?? false) })
        assert(!storageFacts.contains { $0.factType == "SwiftUserDefaultsKeyAccessed" && ($0.properties["normalizedKey"]?.hasPrefix("tenant-") ?? false) })
