...
| Visual Basic syntax fallback | `vb-syntax/0.3.9` | Tier3 (facts), Tier4 (parse/read/budget/semantic-unavailable/event/phase-failure gaps) | `vb.syntax.declarations.v1`, `vb.syntax.memberaccess.v1`, `vb.syntax.invocation.v1`, `vb.syntax.callgraph.v1`, `vb.syntax.objectcreation.v1`, `vb.syntax.event-wiring.v1`, `vb.syntax.database-operation.v1`; syntax call edges carry the explicit `syntax-only` coverage label, invocations and receiver creations are retained together in source order under the per-file budget so field/local initializers are not starved by high call volume, and explicit supported ADO.NET data-adapter `Fill` receivers produce reduced syntax-only database-operation candidates without retaining SQL text |
