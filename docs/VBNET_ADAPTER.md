...
The syntax fallback also retains each method block's source line bounds. Those
bounds are location evidence only; a separate exact document-checksum and PDB
method-row join is required before a projectless method can enter compiled IL.
