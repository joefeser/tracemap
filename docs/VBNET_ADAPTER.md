...
| Shared Web Forms extractor | `legacy-webforms/0.9.1` | Tier1/Tier2/Tier3 facts and Tier4 gaps | existing `legacy.webforms.*` contracts, now with bounded VB code-behind/designer parsing, inline client-behavior candidates, projectless VB server navigation/request-lifecycle/control-state candidates, and shared database/WCF/ASMX terminal projection |
