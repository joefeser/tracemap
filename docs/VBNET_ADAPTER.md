...
| Visual Basic syntax fallback | `vb-syntax/0.3.10` | Tier3 (facts), Tier4 (parse/read/budget/semantic-unavailable/event/phase-failure gaps) | `vb.syntax.declarations.v1`, `vb.syntax.memberaccess.v1`, `vb.syntax.invocation.v1`, `vb.syntax.callgraph.v1`, `vb.syntax.objectcreation.v1`, `vb.syntax.event-wiring.v1`, `vb.syntax.database-operation.v1`; syntax call edges carry the explicit `syntax-only` coverage label, invocations and receiver creations use deterministic round-robin admission across containing members under the per-file budget so one noisy method cannot starve field/local initializers or later method bodies, and explicit supported ADO.NET data-adapter `Fill` receivers produce reduced syntax-only database-operation candidates without retaining SQL text |
