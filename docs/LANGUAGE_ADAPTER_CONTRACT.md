...
| `WebFormsHandlerResolved` | Links event-source identities to scoped code-behind methods, preferring canonical semantic method IDs when proven. Identifier-valued unfamiliar `On...` server attributes may resolve a linked method while retaining reduced event-candidate coverage; this does not prove the custom framework event contract or runtime binding. | `surfaceIdentity`, `controlIdentity`, `eventSourceIdentity`, `handlerSymbolId`, `sourceSymbolId`, `bindingFactId`, `resolutionKind` |
...
| `SymbolRelationship` under `legacy.data.edmx.symbol-composition.v1` | Composes compiler-resolved EF6 CLR entity/property symbols with EDMX descriptors through the deterministic namespace ladder (`MapsToConceptualEntity`, `MapsToConceptualProperty`, `MapsToStorageTable`, `MapsToStorageColumn`). | `relationshipKind`, `relationshipSource`, `sourceSymbolId`, `targetSymbolId` (stable model key), `targetSymbolLanguage=edmx`, `namespaceBridgeFactId`, `namespaceBridgeMechanism`, `supportingFactIds`, `coverageLabel`, `limitations` |
| `LegacyDataGeneratedFileScope` | Records the composition-owned generated-file scope bridge: same-directory exact-base designer files for one EDMX; file scoping only, never CLR identity. | `scopeRule=same-directory-designer-file`, `scopedFilePaths`, `sourceMetadataFactId`, `coverageLabel` |
...
EF6 EDMX symbol-composition edges are capped at `Tier2Structural` (weakest supporting link: the CLR declaration, the explicit namespace-bridge fact, and the Tier2 EDMX descriptors) and never upgrade descriptor facts, generated-code links, or downstream classifications. Every composed edge requires `namespaceBridgeFactId` within its ordered `supportingFactIds`, with `namespaceBridgeMechanism` of `semantic-attribute`, `generation-metadata`, or `generated-file-scope`. Reconciliation is namespace-qualified with single canonical-symbol/compilation-scope uniqueness; global short-name matching, display labels as identity, and generated-link facts as identity authority are prohibited. Uncovered generated-file scopes report `ClrSymbolEvidenceUnavailable` so failed analysis is never presented as missing generated code; the opt-in reverse-impact `mapping` filter traverses these edges upstream with hop-level supporting-fact and bridge provenance and bounded contained-member expansion for entity types reached mid-traversal.
