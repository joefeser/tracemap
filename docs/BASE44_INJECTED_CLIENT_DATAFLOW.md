...
1. the helper resolves to a local function declaration or exported local
   function imported by name/default;
...
targets. Cycles without a source SDK binding do not become authoritative.
...
parameter. A nested function parameter shadows an outer injected binding.
...
identities. Raw argument values and source snippets are not persisted.
