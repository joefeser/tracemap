...
| Type forwarding | `ILRW-TOPO-003`: `TypeForwardingManagedAssemblyUnsupported` | Forwarded targets are not resolved or joined. |
...
| Missing or mismatched debug evidence | `ILRWPDB-EMBEDDED-MISSING-014` and `ILRWPDB-EMBEDDED-MISMATCH-015`, plus existing PDB binding and reader-disagreement gaps | Withhold the disputed PDB relationship while retaining independent parent IL evidence. |
