...
- Visual Basic facts expose compiler-resolved declaration/reference/call/construction/argument/relationship evidence and bounded syntax-only fallback with rule IDs, evidence tiers, repository-relative spans, commit SHAs, extractor versions, and honest reduced coverage; per-file syntax fallback never duplicates Tier1-covered declarations, while individual unresolved call sites in otherwise semantic files may retain explicitly Tier3 evidence.
- Visual Basic scans retain compiler-resolved or explicitly syntax-only event evidence for `Handles`, `AddHandler`, `RemoveHandler`, `RaiseEvent`, and `WithEvents` without representing event wiring as executed call edges; `RemoveHandler` never becomes a positive binding, ambiguous or late-bound sites remain gaps, syntax-only text never becomes a compiler-resolved target, and VB-scan symbol identities are not joined to C# identities across languages.
