...
- [ ] 4. Add report model and deterministic output. Requirements: 7, 8, 10.
  ...
  - [ ] Add `--format`, `--scope`, `--source`, `--change-id`, `--kind`, `--surface`, `--endpoint`, cap flags, and `--exit-code` behavior.
  - [ ] Add directory/file output behavior matching existing report conventions, including directory output writing both Markdown and JSON.
...
- [ ] 5. Implement single-index matchers. Requirements: 2, 4, 6, 10.
  ...
  - [ ] Match endpoint references against route/client endpoint facts using exact `normalizedPathKey` plus method for strong matches.
  - [ ] Match package references against manifest/package/config/import/usage facts where present.
...
- [ ] 7. Implement combined-index support. Requirements: 3, 4, 6, 8, 10.
  ...
  - [ ] Preserve source labels, source index IDs, repo identity hash, scan ID, commit SHA, language, analysis level, and build status.
...
- [ ] 8. Add optional combined-only path/reverse context. Requirements: 5, 6, 9, 10.
  ...
  - [ ] Reuse existing combined path/reverse query services without mutating indexes.
  - [ ] Enforce caps for paths, reverse roots, evidence rows, gaps, and total context queries.
...
- [ ] 10. Validate implementation. Requirements: 10.
  ...
  - [ ] Combined smoke checks if path/reverse/combined behavior changes.
