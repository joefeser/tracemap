...
6. WHEN files are under `.git`, build outputs, DerivedData, `.build`, Pods build outputs, Carthage checkouts/build outputs, SwiftPM checkouts, dependency caches, generated TraceMap output, or hidden tool caches THEN they SHALL be excluded by default unless explicitly included.
