...
Implementation is complete for the v0 slice when this branch lands. Keep tasks checked to reflect shipped code, tests, validation, and PR-loop evidence.

## Phase 0: Scope And Contracts

- [x] Confirm implementation branch name:
  `codex/implement-swift-package-dependency-surfaces`.
- [x] Confirm this implementation-state file exists with initial branch, scope,
...
- [x] Confirm the implementation starts from latest `origin/dev` after #381 is
  merged.
- [x] Confirm `swift run --package-path src/swift tracemap-swift-smoke-tests`
  ...
  before continuing.
- [x] Confirm no package manager restore/build/registry/network operation is in
  scope.
...
- [x] Add rule catalog entries for:
...
- [x] Add `swift.dependency.surface.v1` only as a planned/deferred rule with
  ...
  composition.
- [x] Update `rules/rule-catalog.yml` header to document status field vocabulary
...
- [x] Document limitations for SwiftPM, CocoaPods, Carthage, lockfiles, dynamic
  metadata, unsafe identity values, and absent runtime/build proof.
- [x] Define dependency fact/property model and stable identity inputs.
- [x] Add or reuse safe-value helpers for dependency identities, locations,
  versions, revisions, and source kinds.
- [x] Reuse existing safe-label predicates or normalization behavior where
...
- [x] Document that existing Swift inventory facts are aggregate/count-level
  handoff evidence and new dependency facts are per-row evidence that may cite
  aggregate facts via `supportingFactIds`.
- [x] Define per-source lockfile evidence tiers:
...
- [x] Define closed vocabularies for dependency identity, version, revision,
  source-location, Cartfile `sourceKind`, and metadata status properties.
- [x] Emit aggregate inventory facts before dependency surface facts, omit
...
- [x] Document sorted repo-relative file processing order before per-file
  parser-event occurrence indexes are assigned.
- [x] Document byte-offset anchor tokens for occurrence indexing:
  `.package(`, `pod`, `github`/`git`/`binary` source-kind tokens, and
  lockfile JSON `identity`/`package` keys.
- [x] Document full 64-character SHA-256 dependency hash format and assert
  stable keys/fact IDs never contain raw URLs, hosts, or local paths.
- [x] Do not emit `swift-dependency-metadata-too-large` until the exact
  file-size or dependency-count threshold is documented in
  `rules/rule-catalog.yml` and covered by a boundary fixture.
- [x] Add tests proving raw URLs, hostnames, local paths, credentials, raw
  snippets, raw remotes, and unsafe labels do not appear in generated artifacts.

## Phase 2: Fixtures And Test Harness

- [x] Add SwiftPM fixture data for static `Package.swift` dependency
  declarations and supported `Package.resolved` v1/v2 pins.
- [x] Add CocoaPods fixture data for static `Podfile` declarations and
  parseable `Podfile.lock` sections.
- [x] Add Carthage fixture data for static `Cartfile` and
  `Cartfile.resolved` entries.
- [x] Add or extend a checked-in sample such as
...
- [x] Add malformed/dynamic/unsupported metadata fixtures for each package
  manager.
- [x] Add duplicate dependency fixture rows proving deterministic distinct fact
  IDs and no arbitrary winner behavior.
- [x] Add unsafe same-line dependency rows proving parser-event occurrence
  discriminators are deterministic and distinct.
- [x] Interpret the unsafe same-line fixture as two dependency entries that
  ...
  collision coverage.
- [x] Add `Package.resolved` schema v3 fixture proving unsupported schema emits
  a gap rather than a crash.
- [x] Add a unit test asserting the schema v3 fixture emits exactly the
...
- [x] Ensure the schema v3 fixture is structurally valid JSON with a v3 schema
  marker such as `"version": 3`, so the test exercises unsupported schema
  handling rather than malformed JSON handling.
- [x] Add the same schema v3 assertion to the Swift smoke executable as
  validation backstop.
- [x] Add local-path `.package(path:)` fixture proving raw local paths are
  omitted and gap/status evidence is emitted.
- [x] Add empty-pins `Package.resolved` v1 or v2 fixture proving zero lockfile
  entry facts, zero lockfile gaps, and no coverage downgrade.
- [x] Add Cartfile `binary` fixture proving unsafe source location handling.
- [x] Assert binary-source Cartfile facts use `sourceLocationStatus` of
  `hashed` or `unsafe-omitted` and never `safe`.
- [x] Add Carthage `github "AcmeSecret/PaymentsSDK"`-style fixture proving
...
- [x] Add mixed safe/dynamic Podfile fixture proving safe rows and gap rows
  coexist.
- [x] Add multi-line pod declaration fixture, for example `pod 'Alamofire',`
  ...
  option line.
- [x] Add duplicate `SPEC CHECKSUMS` pod-name fixture proving exactly one
...
- [x] Add zero-dependency metadata fixture proving reports do not imply an
  error or clean absence beyond the existing coverage label.
- [x] Add aggregate-vs-per-row report fixture proving dependency counts are not
  double-counted.
- [x] Add ordering assertion proving aggregate inventory facts appear before
  `SwiftDependencyDeclared` and `SwiftDependencyLockfileEntryDeclared` facts in
  `facts.ndjson` within the same scan.
- [x] Ensure the ordering fixture contains at least one dependency row whose
  ...
  ordering.
- [x] Add supportingFactIds stability assertion across identical scans.
- [x] Add supportingFactIds absence assertion when an aggregate inventory fact
  is absent from the same scan.
- [x] Add context-aware forbidden-word fixture that renders
...
- [x] Add closed-vocabulary tests for `dependencyIdentityStatus`,
...
- [x] Add sourceSection safety test proving raw unsafe section headings are
  hashed or omitted before emission.
- [x] Add zero-static-dependency `Package.swift` fixture proving no false clean
  absence claim.
- [x] Add duplicate `Package.resolved` identity fixture proving deterministic
  distinct rows and one `swift-dependency-lockfile-malformed` gap for the
  duplicate-pin ambiguity.
- [x] Add test proving `normalizedDependencyIdentity` is absent when
  `dependencyIdentityStatus` is `hashed` or `unsafe-omitted`.
- [x] Confirm `tracemap-swift-smoke-tests`, created by the prerequisite Swift
...
- [x] Add byte-stability tests proving dependency facts are unchanged when only
  output path changes after Phase 3-5 emit dependency facts.

## Phase 3: SwiftPM Dependency Surfaces

- [x] Parse conservative static `.package(...)` declarations from
  `Package.swift` without executing Swift.
- [x] Emit `SwiftDependencyDeclared` facts for safe SwiftPM manifest
  dependencies.
- [x] Emit `SwiftDependencyLockfileEntryDeclared` facts for supported
  `Package.resolved` pins.
- [x] Treat `Package.resolved` schema v3 and later as unsupported until
  explicitly specified and emit a dependency-specific gap.
- [x] Hash or omit unsafe SwiftPM URLs, hostnames, local paths, revisions, and
  branches.
- [x] For URL-only SwiftPM dependencies, derive a candidate identity from the
...
- [x] Emit dependency-specific gaps for dynamic manifests, unsupported shapes,
  malformed lockfiles, unsupported schemas, and unsafe identities.
- [x] Add tests for safe rows, unsafe rows, malformed rows, duplicate rows, and
  report rendering.

## Phase 4: CocoaPods Dependency Surfaces

- [x] Parse conservative static `pod` declarations from `Podfile` without
  executing Ruby.
- [x] Emit `SwiftDependencyDeclared` facts for safe CocoaPods declarations.
- [x] Parse common `Podfile.lock` `PODS` and `DEPENDENCIES` sections.
- [x] Emit `SwiftDependencyLockfileEntryDeclared` facts for safe lockfile
  entries.
- [x] When a pod appears in both `PODS` and `DEPENDENCIES`, emit one lockfile
  entry fact per section occurrence rather than merging the rows.
- [x] Skip `SPEC REPOS`, `COCOAPODS`, and unknown sections unless the file is
  otherwise malformed; read `SPEC CHECKSUMS` only for checksum presence/count
  or an aggregate hash over sorted pod names.
- [x] For duplicate `SPEC CHECKSUMS` pod-name keys, emit one
  `swift-dependency-lockfile-malformed` gap and deduplicate the raw key before
  computing `podChecksumSectionHash`.
- [x] Treat `source`, `target`, `platform`, `use_frameworks!`, `plugin`, and
  similar Podfile configuration lines as out of scope for v0 dependency rows.
- [x] Hash or omit git/path/source options and unsafe external values.
- [x] Emit gaps for dynamic Ruby, unsupported sections, malformed lockfiles,
...
- [x] Parse conservative static `Cartfile` entries for `github`, `git`,
  `binary`, and other documented source kinds.
- [x] Emit `SwiftDependencyDeclared` facts for safe Carthage declarations.
- [x] Emit `sourceKind=github`, `git`, `binary`, or `unknown` for Cartfile
  manifest rows while keeping `declarationKind=cartfile-declaration`.
- [x] Parse supported `Cartfile.resolved` entries.
- [x] Emit `SwiftDependencyLockfileEntryDeclared` facts for safe resolved rows.
- [x] Persist clean semver version strings when safe, and reduce non-semver
  revisions, branches, URLs, and unsafe values to status flags plus hashes.
- [x] Emit `revisionStatus=absent` for manifest Cartfile rows that contain only
  a version constraint.
- [x] Treat manifest `Cartfile` version ranges such as `~> 1.0` as
  `versionStatus=present` with the raw range hashed, not rendered.
- [x] Hash or omit unsafe external locations and versions/revisions where
  needed.
- [x] Emit gaps for malformed, unsupported, unsafe, or ambiguous Carthage
  metadata.
- [x] Add tests for safe rows, unsafe rows, malformed rows, duplicate rows, and
  report rendering.
...
- [x] Defer `SwiftDependencySurfaceDeclared` in this slice. If a planned
  catalog entry is added, mark/document it as planned/deferred and assert no
  emitted fact uses the rule.
- [x] Treat `SwiftDependencySurfaceDeclared` as active only when
...
- [x] Implement the `stableDependencySurfaceKey` suppression as a code-level
...
- [x] Add a smoke assertion that no emitted `facts.ndjson` row uses
  `ruleId = swift.dependency.surface.v1` while the composed surface is deferred.
- [x] Add an assertion that `stableDependencySurfaceKey` is absent from all
  emitted facts, including dependency rows and `AnalysisGap` rows, while
  composed surfaces are deferred.
- [x] Record the future composition rule: if emitted later, compose only
...
- [x] Persist dependency facts in `facts.ndjson` and `index.sqlite` with sorted
  properties and stable fact IDs.
- [x] Add report sections or summary rows for Swift dependency counts by
  package manager, metadata kind, identity status, and gaps.
- [x] Add report/export tests proving aggregate inventory counts and per-row
  dependency facts are not double-counted.
- [x] Identify the Swift local report and .NET combined dependency-report
...
- [x] Verify report wording uses static dependency metadata language only.
- [x] Add forbidden-word assertions for report output: no unsupported claims of
...
- [x] Include a forbidden-word fixture that renders rule IDs such as
...
- [x] Verify `.NET` `export`, `combine`, and `report` preserve dependency
  facts, rule IDs, evidence tiers, source labels, commit SHA, and gaps.
- [x] Add focused tests or smoke assertions for generated public-safety and
  deterministic output.

## Phase 8: Documentation And Validation

- [x] Update `docs/VALIDATION.md` with any new Swift fixture commands or report
  checks.
- [x] Update `docs/LANGUAGE_ADAPTER_CONTRACT.md` only if a shared dependency
  surface contract is introduced.
- [x] Before Phase 7 implementation merges, record in implementation-state
  ...
  section.
- [x] Update `docs/ACCEPTANCE.md` only if public acceptance claims change.
- [x] Keep public/site wording bounded to static checked-in metadata evidence.
- [x] Record implementation decisions, validation, and follow-ups in
  `.kiro/specs/swift-adapter-v0-package-dependency-surfaces/implementation-state.md`.

## Phase 9: Validation

- [x] Run Swift adapter validation:
...
- [x] Run shared reader validation:
...
- [x] Run core validation when shared reader behavior changes:
