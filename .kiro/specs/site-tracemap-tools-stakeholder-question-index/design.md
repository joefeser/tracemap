...
| Question family | Primary audience | Example safe question | Target route | Evidence surface | Public claim level | Safe answer shape | Proof path | Rule ID/rule family | Limitation | Non-claim |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Manager planning | Managers, leads | What public-safe evidence can I bring into planning without overclaiming? | `/manager-packet/` | Manager packet, proof paths, and limitations. | concept | Inspect manager-facing summaries, proof paths, and limitations. | `/manager-packet/`, `/proof-paths/`, `/limitations/` | Rule family or surface label when no specific public-safe rule ID is exposed. | Does not decide staffing, priority, ownership, release readiness, or operational safety. | Does not replace manager judgment or service-owner decisions. |
| Engineer endpoint/change review | Engineers, reviewers | Where can I inspect static evidence around this endpoint or change before review? | `/use-cases/endpoint-review/` | Endpoint/change review route, demo result, and validation boundaries. | concept | Review static endpoint/change evidence and validation boundaries. | `/use-cases/endpoint-review/`, `/demo/result/`, `/validation/` | Specific route/endpoint rule IDs when public-safe; otherwise endpoint evidence rule family. | Does not prove the change is safe, unsafe, approved, blocked, or production-proven. | Does not replace tests, code review, telemetry, or service-owner review. |
| Incident-adjacent handoff | Incident participants, service owners | What static evidence can I hand to the runtime or incident owner without claiming cause? | `/incident-evidence-handoff/` | Incident handoff, static-versus-runtime, and limitations surfaces. | concept | Route static context to runtime owners while preserving gaps. | `/incident-evidence-handoff/`, `/static-vs-runtime/`, `/limitations/` | Static evidence handoff rule family unless a public-safe rule ID is exposed. | Does not prove outage cause, timeline, traffic, performance, or runtime behavior. | Does not replace incident command, telemetry, logs, traces, or human judgment. |
| Modernization planning | Architects, modernization planners | What evidence map can help plan modernization questions while preserving gaps? | `/legacy-modernization/evidence-map/` | Modernization evidence map, proof paths, and validation. | concept | Inspect modernization evidence maps and known gaps. | `/legacy-modernization/evidence-map/`, `/proof-paths/`, `/validation/` | Legacy modernization or dependency evidence rule family unless a public-safe rule ID is exposed. | Does not prove complete migration scope or complete dependency understanding. | Does not replace architects, service owners, tests, or modernization planning. |
| Reviewer claim checking | Reviewers, managers, agents | Can this sentence be repeated, and what proof must stay attached? | `/review-claim-checklist/` | Claim checklist, proof paths, and proof-source catalog. | concept | Check whether a claim keeps its proof fields and limitations attached. | `/review-claim-checklist/`, `/proof-paths/`, `/proof-source-catalog/` | Rule ID when the claim cites one; otherwise rule family plus limitation. | Does not upgrade a claim by repetition, seniority, or confidence. | Does not allow dropping rule ID/rule family, tier, coverage, proof path, limitation, or non-claim. |
| Demo evaluation | Evaluators, engineers, managers | What does the public demo show, and which limits still apply? | `/demo/result/` | Demo result, proof paths, and limitations. | concept | Compare demo rows with proof paths and limitations. | `/demo/result/`, `/proof-paths/`, `/limitations/` | Demo row rule IDs when public-safe; otherwise demo evidence rule family. | Does not prove private repo behavior, runtime behavior, release safety, or complete coverage. | Does not turn demo evidence into production proof. |
| Proof-source inspection | Reviewers, bots, engineers | Where does this public claim point for source-of-truth evidence? | `/proof-source-catalog/` | Proof-source catalog, proof paths, and validation. | concept | Inspect the public-safe source mapping behind a claim. | `/proof-source-catalog/`, `/proof-paths/`, `/validation/` | Rule ID or rule-family field transcribed from the proof-source row when available. | Does not publish raw facts, SQLite, logs, snippets, SQL, configs, private samples, remotes, or scan dirs. | Does not make local-only artifacts public proof. |
| Agent/bot discovery | Crawlers, automated reviewers | Which routes and metadata should an agent use without dropping proof fields? | `/proof-paths/` | Sitemap/discovery metadata plus proof, validation, and limitation routes. | concept | Follow metadata and proof routes while preserving evidence fields. | `/proof-paths/`, `/validation/`, `/limitations/` | Rule ID or rule family from the target proof route; discovery metadata alone is not a rule source. | Does not authorize autonomous approval, claim upgrades, or hidden-detail disclosure. | Does not imply AI impact analysis or permission to repeat claims without limitations. |

Implementation may adjust the target routes to current equivalents, but it
must preserve the question family, audience, evidence surface, safe answer
shape, rule ID/rule family handling, limitation, and non-claim for every
required row.
...
- `proof path`: public-safe link chain or route that preserves rule ID or rule
  ...
  available.
- `rule ID/rule family`: specific rule ID when public-safe and relevant;
  otherwise a rule-family or evidence-surface label plus a limitation.
