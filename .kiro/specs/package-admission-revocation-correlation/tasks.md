...
- [x] 4.4 Adapter validation per `docs/VALIDATION.md`: `npm run check --prefix src/typescript`; pinned TypeScript smokes; `python3 scripts/validate-adapter-artifacts.py <scan-output>`; combined combine/report/paths smoke. The reviewed `axios-npm-lock` fixture is pinned to `84a9f3b9a4f3244b8c8e818f557d64c7b964fb25`, ships `package-lock.json` v3, passes the shared validator with no unsafe/private-path findings, and exercises exact digest correlation plus typed unavailable path/reverse context; see `implementation-state.md`.
