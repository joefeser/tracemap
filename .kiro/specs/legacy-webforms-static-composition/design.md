...
- Client-script methods are a closed representative set: `RegisterStartupScript`, `RegisterClientScriptBlock`, `RegisterClientScriptInclude`, `RegisterOnSubmitStatement`, and `RegisterHiddenField`. The invocation must use a supported Page, ClientScript, or ScriptManager receiver and a documented receiver/method signature. Positional and named arguments must map uniquely to that signature before a literal payload is hashed; same-named helpers, local or containing-member receiver shadows, aliases or file-local custom types with supported receiver names, unsupported parameter names/shapes, and dynamic payloads are gaps.
- `__doPostBack` parsing accepts only a standalone JavaScript identifier with a literal first argument. Prefixed identifiers and member-qualified custom functions are ignored. Exact same-surface control matching is optional supporting evidence, never a dispatch claim.
