...
| `combined_dependency_edges` | persisted static dependency edge IDs when available; PR 1 must not assume report-generated message candidate edges are persisted here |
...
`extractorVersions[]` should be populated from the safest available source
metadata in the derived artifact, such as scan manifest or source-level
extractor metadata. If the current combined-report row shape does not expose
per-fact extractor versions, the field must still be present as an empty array
or source-level best effort, and the row must carry a caveat rather than
inventing per-fact versions.
...
Classification severity order for sorting, highest first:
...
3. Populate rows from already projected message surfaces and report-generated
   `message-publish-consume` candidate edges. For the recommended
   `tracemap report` consumer, use the candidate edges generated by the
   combined report path, or the same helper that generates them, instead of
   requiring those candidate edges to be persisted in `combined_dependency_edges`.
