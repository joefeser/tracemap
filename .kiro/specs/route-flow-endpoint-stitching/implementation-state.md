...
criteria used by endpoint composition. Non-endpoint roots report conservative
fallback states such as `path-node` or `symbol-fallback`.
...
- Added focused assertions for successful `method-symbol` bridge state,
  missing bridge state, additive JSON deserialization, Markdown output, and
...
- `dotnet test src/dotnet/TraceMap.sln`: passed, 604 tests, with the existing
  `SQLitePCLRaw.lib.e_sqlite3` NU1903 warning.
