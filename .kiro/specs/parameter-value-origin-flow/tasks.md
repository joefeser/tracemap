...
- [ ] 1. Confirm current flow evidence behavior. Requirements: 1, 2, 3, 9.
  - [ ] Inspect .NET `ArgumentPassed`, `LocalAlias`, `FieldAlias`, `ParameterForwardEdge`, flow-boundary, runtime-evidence, and symbol-identity facts.
  ...
  - [ ] Inventory combined report/path/reverse/diff/impact handling of value-origin evidence.

- [ ] 2. Define shared value-origin models. Requirements: 1, 8, 9, 10.
  - [ ] Decide whether new public report models are required or existing path/reverse models can carry value-origin notes.
  - [ ] Define shared safe properties for argument, parameter, origin, constructor, and boundary roles.
  - [ ] Define value-origin classifications as additive metadata/notes while preserving existing path-level classification strings.
  - [ ] Document compatibility behavior for older indexes without precise flow tables.
  - [ ] Audit combined flow table/view definitions before adding schema or changing views.

- [ ] 3. Harden direct argument-to-parameter extraction. Requirements: 2, 8, 11.
  - [ ] Add or update tests for semantic direct forwarding.
...
- [ ] 4. Harden local and member origin evidence. Requirements: 4, 5, 8, 11.
  - [ ] Add same-method local alias tests.
  - [ ] Pin alias chain bound, default `3` hops, in tests and rule limitations.
  - [ ] Add deterministic field/member alias tests.
  - [ ] Add unique constructor parameter-to-member-to-call tests.
  - [ ] Add ambiguous multiple-constructor/multiple-assignment downgrade tests.
...
- [ ] 8. Update rule catalog and docs. Requirements: 1, 8, 10.
  - [ ] Reuse existing rule IDs where behavior is unchanged.
  - [ ] Add new `combined.flow.*.v1` or adapter-local rules only for new evidence behavior.
  - [ ] Document limitations for every new rule.
  - [ ] Update `docs/LANGUAGE_ADAPTER_CONTRACT.md`, `docs/VALIDATION.md`, and `docs/ACCEPTANCE.md` during implementation.
  - [ ] Review new safe metadata keys against private-path guard expectations.

- [ ] 9. Validate. Requirements: 11.
  - [ ] Run affected adapter test suites.
...
- [ ] PR 1: Current-state audit + shared value-origin model + focused .NET tests for direct forwarding and alias boundaries.
