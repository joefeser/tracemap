...
Completed tasks are checked for the implementation PR. Relationship-heavy
tasks owned by issue #381 and broad downstream workflow tasks remain unchecked
...
- [x] 0.8 Check whether #381 has landed. If it has, skip the interim
...
- [x] 0.9 Confirm and document the exact Swift `scanId` input formula from #378
...
- [x] 1.1 Add `rules/rule-catalog.yml` entries before product code emits any
  Swift declaration, call, object creation, navigation, relationship, import,
  or gap rule ID.
- [x] 1.2 Document each Swift rule with the existing catalog fields: `id`,
...
- [x] 1.3 Add or reuse a rule-catalog validation hook so tests fail when a
  Swift fact references an uncataloged rule ID.
- [x] 1.4 Document whether the rule-catalog validation hook is existing shared
  infrastructure or new Swift adapter infrastructure; if it is new, implement
  the hook before Phase 3 begins.
- [x] 1.5 Add a negative catalog-gate test proving an emitted uncataloged Swift
  rule ID fails validation.
- [x] 1.6 Decide which shared fact types can be reused safely: default to weaker
...
- [x] 1.7 Decide whether `TypeDeclared`, `AnalysisGap`, and related
  ...
  `ObjectCreated`.
- [x] 1.8 Add Swift-specific fact types only when shared fact types would
  overstate semantics.
- [x] 1.9 Record an explicit written decision in `implementation-state.md` for
...
- [x] 2.1 Add a minimal Swift fixture with imports, a struct, class, actor,
...
- [x] 2.4 Add deterministic output tests for repeated scans over the same
  fixture and options.
- [x] 2.5 Add public-safety tests proving raw snippets, raw expressions, raw
...
- [x] 2.7 Add scan ID stability tests proving unchanged repo, commit, selected
  Swift file inventory, and normalized options produce the same `scanId`, even
  when `--out` points at a different output path.
- [x] 2.8 Add parser diagnostic safety tests proving SwiftSyntax diagnostic
  message text is hashed or omitted rather than stored verbatim.

### Phase 3: SwiftSyntax Parsing and Span Mapping

- [x] 3.1 Wire SwiftSyntax file parsing into the Swift adapter without running
  app code, SwiftPM builds, Xcode builds, simulators, or devices.
- [x] 3.2 Map SwiftSyntax source locations to repo-relative file paths and
  stable line spans.
- [x] 3.3 Continue after recoverable parse diagnostics and emit bounded
  `AnalysisGap` facts.
...
- [x] 4.1 Emit or preserve Swift file inventory facts with relative path, file
  kind, safe hash/size metadata, rule ID, tier, extractor ID, and extractor
  version.
- [x] 4.2 Attach module/package/target context from the inventory slice when it
  is deterministic.
- [x] 4.3 Emit `ModuleContextUnavailable` or `ModuleContextAmbiguous` gaps when
  context is missing or unsafe.
- [x] 4.4 Emit import facts for normal and scoped import declarations with safe
  module path segments and import kind.
- [x] 4.5 Add tests for conditional imports, scoped imports, ambiguous module
  context, `@_exported` imports, `#if canImport(...)`, and safe import
  metadata.

### Phase 5: Declaration Extraction

- [x] 5.1 Emit declaration facts for classes, structs, actors, enums, protocols,
...
- [x] 5.2 Generate #381-compatible Swift symbol IDs from safe module context,
...
- [x] 5.3 Emit reviewer-friendly display signatures without claiming semantic
  resolution.
- [x] 5.4 Record function/method/init parameter labels, arity, async/throws
  markers, and signature hashes.
- [x] 5.5 Record property shape metadata for stored/computed/accessor/wrapper
...
- [x] 6.1 Emit call facts for simple identifier calls, member calls,
  static-looking member calls, `self`/`super` calls, and chained call sites.
- [x] 6.2 Emit object/construction evidence for initializer-like syntax,
  explicit `.init(...)`, and type construction candidates where the created
  type syntax is safely identifiable.
- [x] 6.3 Attach nearest containing declaration symbol IDs to calls in methods,
  functions, initializers, property initializers, accessors, closures, local
  functions, and result-builder-like bodies.
- [x] 6.4 Store argument labels, arity, expression kinds, and expression hashes
  without storing raw argument text.
- [x] 6.5 Emit gaps or reduced limitations for optional chaining,
...
- [x] 7.1 Populate shared `symbols`, `symbol_occurrences`, `fact_symbols`,
  `call_edges`, `object_creations`, and `symbol_relationships` rows only when
  backing facts exist.
- [x] 7.2 Preserve supporting fact IDs for every derived row.
...
- [x] 7.4 Keep syntax call/navigation edges capped at `Tier3SyntaxOrTextual`.
- [x] 7.5 Emit gaps instead of strong edges for protocol dispatch, overload
  ...
  reflection.
- [x] 7.6 Add SQLite and export/combine compatibility tests for Swift symbols,
  relationships, call edges, object creations, and supporting fact IDs.
- [x] 7.7 Add an assertion that every derived `call_edges`,
  `object_creations`, and `symbol_relationships` row has a supporting fact ID.
- [x] 7.8 Add a tier-cap assertion that Swift syntax call, navigation, and
...
- [x] 7.10 Add Tier2 downgrade tests proving `ModuleContextUnavailable` or
  `ModuleContextAmbiguous` keeps SwiftSyntax facts at `Tier3SyntaxOrTextual`.

### Phase 8: Report and Downstream Compatibility

- [x] 8.1 Update Swift scan report sections with declaration counts, call
  counts, object creation counts, relationship counts, module context status,
  reduced-coverage gaps, and limitations.
- [x] 8.2 Verify report wording says syntax-backed, static, candidate,
  unresolved, or gap as appropriate.
- [x] 8.3 Add forbidden-wording tests so reports and exports do not claim
...
- [x] 8.7 Add forbidden-wording coverage proving `@_exported import` facts do
...
- [x] 9.4 Run the Swift adapter build command selected by the scaffold spec.
- [x] 9.5 Run the Swift adapter test command selected by the scaffold spec.
- [x] 9.6 Run a Swift fixture scan and verify required artifacts:
  `scan-manifest.json`, `facts.ndjson`, `index.sqlite`, `report.md`, and
  `logs/analyzer.log`.
- [x] 9.7 Run `dotnet build src/dotnet/TraceMap.sln` when shared .NET
  readers/schema/report behavior changes.
- [x] 9.8 Run `dotnet test src/dotnet/TraceMap.sln` when shared .NET
  readers/schema/report behavior changes.
- [x] 9.9 Run relevant `docs/VALIDATION.md` smokes for adapter, combine,
  report, paths, reverse, diff, impact, release-review, or export behavior
  touched by the implementation.
- [x] 9.10 Run `./scripts/check-private-paths.sh`.
- [x] 9.11 Run `git diff --check`.
