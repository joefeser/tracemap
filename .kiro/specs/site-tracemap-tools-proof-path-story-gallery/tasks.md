...
- [ ] Record selected and rejected placements in `implementation-state.md`.
- [ ] Explain why the selected placement is a story gallery rather than the
  canonical proof ledger, proof-source catalog, claim checklist, roadmap, or
  limitations page.
- [ ] Add the concept-level public gallery page or section using existing
  static-site layout, accessibility, metadata, and validation patterns.
- [ ] Include visible `Public claim level: concept` unless every card has
...
- [ ] If implemented as a folded section, record host metadata reconciliation
  and add stable unique anchors for every required section.
- [ ] Include required sections: story contract, proof path anatomy, evidence
...
- [ ] Include required story-card fields: static question, story category,
...
- [ ] Ensure evidence packet references include packet/report-family label,
  ...
  when applicable.
- [ ] Ensure stop conditions include `no public-safe evidence`,
...
- [ ] Ensure stop routing uses generic owner/question labels and does not
...
- [ ] Keep gallery conclusions bounded to deterministic static evidence and do
...
- [ ] Avoid "impacted" wording unless tied to public-safe reducer evidence,
  status, rule IDs, and limitations.
- [ ] Do not publish raw facts, SQLite contents, analyzer logs, source
...
- [ ] Add focused validation for visible claim level and shared principle.
- [ ] Add focused validation for required sections and stable anchors.
- [ ] Add focused validation for story-card fields, walkthrough fields,
  evidence references, stop conditions, owner routing, and story categories.
- [ ] Add focused validation for standalone metadata, sitemap metadata,
