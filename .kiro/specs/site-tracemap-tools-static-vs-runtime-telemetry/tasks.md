...
- [ ] Choose the future page placement: standalone `/static-vs-runtime/` route,
...
- [ ] Add a scannable comparison of static evidence questions and runtime
  observability questions.
- [ ] Include static evidence examples such as repository snapshot, commit SHA,
...
- [ ] Add a workflow section for before runtime review, during handoff, and
  after runtime review that keeps runtime owners and service owners responsible
  for operational conclusions.
- [ ] Add manager/reviewer guidance for reading rule IDs, evidence tiers, file
...
- [ ] Ensure copy does not imply TraceMap replaces logs, traces, APM,
...
- [ ] For local-only SQLite, facts, reports, rule catalog material, or analyzer
  logs, name only the artifact family and link only to a public-safe summary or
  route.
- [ ] Ensure no page copy, metadata, alt text, captions, or generated discovery
...
- [ ] Add stable section anchors for static questions, runtime questions,
...
- [ ] If the content is implemented as a section, add a stable in-page anchor
  so cross-links and discovery entries can deep-link without implying a
  standalone shipped route.
- [ ] Add page title, description, canonical URL, Open Graph fields, sitemap
  metadata, and discovery metadata if a standalone route is added.
- [ ] Ensure discovery metadata and any LLM or bot-oriented discovery entry
  preserve the concept claim level and operational non-claims.
- [ ] Use the existing `site/src/_site/discovery.json` `site-page` field shape
...
- [ ] Record any moved or unavailable target route in `implementation-state.md`
...
- [ ] If a section placement was chosen, validate that host-page title,
