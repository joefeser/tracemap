...
- [ ] Choose final placement from `/evidence/gaps/`, `/coverage/gaps/`,
  section on `/limitations/reduced-coverage/`, section on
  `/reviewer-quickstart/`, or a recorded equivalent if site information
  architecture has changed.
- [ ] Before editing site source, enumerate which of the seven adjacent routes
...
- [ ] Explain why the selected placement is a gap register rather than
  `/limitations/reduced-coverage/`, `/limitations/`, `/validation/`,
  `/questions/objections/`, `/owners/follow-up/`,
...
- [ ] Include required sections for when a gap is useful, gap register fields,
  example gap rows, stop conditions, next-owner handoff, safe wording, unsafe
  wording, and non-claims.
- [ ] Include the required missing proof path row.
- [ ] Include the required reduced coverage row.
- [ ] Include the required `Tier4Unknown` row.
- [ ] Include the required private-only support row.
- [ ] Include the required stale commit row.
- [ ] Include the required unsupported framework surface row.
- [ ] Include the required missing validation evidence row.
- [ ] Include the required unresolved owner question row.
- [ ] Ensure every required row includes gap label, what evidence exists, what
  cannot be concluded, public claim level, next owner, proof/validation route,
  safe wording, and stop condition.
- [ ] Use accessible table semantics or an equivalent programmatic repeated
...
- [ ] Link to public-safe equivalents for `/limitations/reduced-coverage/`,
...
- [ ] Forbid absence-of-impact proof, runtime proof, production traffic proof,
...
- [ ] Avoid blame language in row labels, examples, and owner handoff copy.
...
- [ ] Add focused validation for accessible table semantics or equivalent
  ...
  structure.
- [ ] Add focused validation for required links, allowed public-safe target
  families, and deferred/substituted/omitted route cap.
- [ ] Add focused validation that every adjacent-surface link and every
...
- [ ] If the register is implemented as a section on an adjacent surface,
...
- [ ] Add focused validation for adjacent-surface distinctions.
- [ ] Before writing discovery or bot-oriented validators, enumerate and
...
- [ ] Add focused validation for forbidden live claims and unsafe wording
  context.
...
- [ ] Add focused validation for word count bounds: 900 to 1,700 visible body
...
- [ ] Wire focused validation into the existing aggregate site validation
  workflow.
- [ ] Run `npm run build` from `site/` after site source is added to produce
  generated output.
- [ ] Run `npm run validate` from `site/` after build so metadata, sitemap,
  discovery, rendered wording, and private/raw material checks inspect
  generated output rather than only source files.
- [ ] Run `npm test` from `site/` after build.
- [ ] Run desktop and mobile browser sanity checks if route, layout, table,
  responsive behavior, or interaction changes are made.
- [ ] Update this spec's `tasks.md` checkboxes as implementation tasks are
  completed.
- [ ] Keep generated output out of git unless a future workflow explicitly
