...
These tasks track the completed public-site implementation phase for the site
claim guardrails route. Generated output, scanner code, reducer code, runtime
telemetry, and unrelated specs remain out of scope.
...
- [x] Record the selected placement and rejected alternatives in
  `implementation-state.md`.
- [x] Explain why the selected placement is a guardrails surface rather than
  the canonical claim checklist, proof source catalog, roadmap, limitations
  page, objections page, or change-risk language guide.
- [x] Add the concept-level public guardrails page or section, or a hidden
...
- [x] Include visible `Public claim level: concept` for public-facing output.
...
- [x] If implemented as hidden contributor-only docs, keep it out of public
  sitemap, discovery metadata, public `/docs/` links, and navigation, then
  record the hidden rationale.
- [x] If implemented as a section, record host metadata reconciliation and add
  stable unique anchors for every required section.
- [x] Include required sections: public claim levels, proof-path requirements,
...
- [x] In the review handoff section, include bounded next states:
  `repeat with proof`, `downgrade before repeating`,
  `owner follow-up needed`, `do not repeat`, `internal only`, and `hidden`.
- [x] Include all required guardrail rows: shipped, demo, concept, hidden, raw
  artifact reference, dev-only feature, reduced coverage, runtime/release
  wording, AI/LLM wording, and private-only support.
- [x] For every row, include condition, allowed public wording or action,
  required proof path, downgrade or hidden trigger, forbidden implication, and
  review handoff.
- [x] Keep public claim level values limited to `shipped`, `demo`, `concept`,
  and `hidden`.
- [x] Ensure `shipped` requires main-true or source-document proof plus
  limitation and does not imply runtime, release, safety, complete coverage,
  or replacement of human review.
- [x] Ensure `demo` requires checked-in public demo proof or public-safe demo
  summary plus limitation and does not imply shipped or production behavior.
- [x] Ensure `concept` stays future-facing, explanatory, dev-only, process, or
  not-yet-backed and cannot be presented as shipped or demo-backed.
- [x] Ensure `hidden` means omitted or abstracted and does not reveal hidden
...
- [x] Ensure raw artifact references are replaced by public-safe summaries or
  owner follow-up.
- [x] Ensure dev-only features are treated as concept or hidden and do not
  imply public availability.
- [x] Ensure reduced coverage remains visible and forces downgrade or owner
  follow-up unless the claim is narrowed.
- [x] Ensure runtime/release wording is removed or rewritten.
- [x] Ensure AI/LLM wording is removed or rewritten for core scanner/reducer
  behavior.
- [x] Ensure private-only support stays internal, hidden, or owner follow-up
...
- [x] Link to `/proof-source-catalog/` when it exists and explain that it
  remains route-to-source mapping.
- [x] Link to `/roadmap/` when it exists and explain that it remains claim
...
- [x] Add focused validation for visible concept claim level or recorded
  hidden contributor-only rationale.
- [x] Add focused validation for visible shared principle.
- [x] Add focused validation for required sections and stable anchors.
- [x] Add focused validation for required guardrail rows and row fields.
...
- [x] Add focused validation that hidden contributor-only placement is absent
  from public sitemap and discovery metadata if that placement is chosen.
- [x] Add focused validation that hidden contributor-only placement is not
  linked from public `/docs/` if that placement is chosen.
- [x] Add focused validation that the page or section is absent from primary
...
- [x] Add focused validation for no blame language using a recorded advisory
  phrase set.
- [x] Add focused validation that rendered body word count stays between 700
