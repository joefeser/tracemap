...
- Representative result: 7,559 facts; `AccessBindingExpressionPartial` 250 -> 32, `AccessBindingTargetUnresolved` 292 -> 98, `AccessDesignInputCatalogStableKeyUnmatched` 566 -> 0, and one exact `AccessBindingExpressionTargetAmbiguous` gap. Independent post-review reruns preserved those counts and byte-identical facts/manifests after tightening ordinal identity, immutable-parent, unresolved-domain, multi-candidate/domain-control ambiguity, and unscoped-filter behavior.
...
- Validation: locked restore passed; initial focused composition/projector/UI tests 37/37; final post-review focused tests 43/43; Access-focused tests 205/205 before review; full build passed with 0 warnings/errors; final post-review full tests 1,085/1,085; changed-file formatting passed; private-path scan passed; `git diff --check` passed.
...
- PR/ACK: PR #578 targets `dev`. ACK-authorized review work patched all correctness findings, dispositioned low-risk style suggestions, and reached a clean mechanical state with zero unresolved threads, pending/failed checks, or actionable findings. The final code head stopped typed at `CURRENT_HEAD_REQUIRED_REVIEW_MISSING`; exact-head required review or an explicit owner decision remains the merge gate.
