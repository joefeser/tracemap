...
- Representative result: 7,559 facts; `AccessBindingExpressionPartial` 250 -> 32, `AccessBindingTargetUnresolved` 292 -> 98, and `AccessDesignInputCatalogStableKeyUnmatched` 566 -> 0. Two independent runs produced identical facts and manifest SHA-256 values. The post-review rerun preserved those counts and deterministic equality after tightening ordinal identity, immutable-parent, unresolved-domain, field/control-ambiguity, and unscoped-filter behavior.
...
- Validation: locked restore passed; initial focused composition/projector/UI tests 37/37; post-review focused tests 42/42; Access-focused tests 205/205 before review; full build passed with 0 warnings/errors; post-review full tests 1,084/1,084; changed-file formatting passed; private-path scan passed; `git diff --check` passed.
