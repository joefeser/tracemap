...
- Scope decision: add no new COM calls. Reuse the bounded saved-query SQL already read by the product reader to recover one direct projection's source field when DAO output-source metadata is absent. DAO/static disagreement retains both candidates and partial coverage.
- Composition decision: one output-to-source fact may resolve a domain criteria field. A domain return that names the source rather than the query's declared alias remains partial as `AccessBindingDomainSelectedFieldAliasMismatch`; it is not promoted to the aliased output.
- Expected representative effect after a fresh Windows base scan: the three active criteria ambiguities become exact source-field lineage; the two active return dependency ambiguities become precise alias-name contradictions. The twelve numeric-versus-`W` crosstab contradictions remain unchanged and require a source/rebuild naming decision.
- Validation: 107 focused projector/composer/reader tests, all 303 Access-filtered tests, and all 1,181 solution tests pass. The solution builds with zero warnings/errors. Changed-file formatting, adapter artifact validation, private-path guard, and diff validation pass.
- `docs/VALIDATION.md` was followed for the macOS Access smoke: focused design-evidence/projector tests, the full Access-filtered suite, solution build/test, and adapter artifact validation passed. The pinned Windows product smoke is deferred until this reader change is merged; it requires a fresh `scan-file` acquisition from the immutable database copy. No new design export, VBE access, query execution, or row read is required.
- Immutable-old-base check: local enrichment from the prior immutable base still emits 8,982 facts byte-identical to the previous exact-head enrichment. This is expected because that base predates the new `AccessQueryOutputSourceCandidate` facts.
- Deferred validation: regenerate the base scan once on Windows from the authorized immutable database copy, then enrich with the same design bundle and refresh the private remainder register. No additional design export, VBA extraction, row read, query execution, or UI rendering is required.
