...
- [x] 1. Add closed-vocabulary bucket tests.
  Requirements: 1, 4, 5.
  - [x] Enumerate `CombinedTerminalSurfaceKinds.All` in a direct mapping-level
    table test.
  - [x] If needed, extract a tiny internal mapping helper or make the current
    mapper internal so tests can cover vocabulary without scanner-like
    fixtures for every surface kind.
  - [x] Assert `sql-query` and `sql-persistence` map to
    `data-surface terminal context` after an exact selected-property bridge.
  - [x] Assert `legacy-data` maps to `legacy-data terminal context`.
  - [x] Assert `package-config` maps to `package/config terminal context`.
  - [x] Assert every current `message-*` value maps to
    `message-surface terminal context`, including `message-unknown`.
  - [x] Assert every current `asmx-*`, `remoting-*`, and exact `wcf-operation`
    value maps to `legacy-communication terminal context`.
  - [x] Assert `asmx-config` and `asmx-metadata` remain
    `legacy-communication terminal context`, not package/config context.
  - [x] Assert future `wcf-*` values do not implicitly inherit the exact
    `wcf-operation` decision without a mapped, suppressed, or deferred
    vocabulary decision.
  - [x] Assert an out-of-vocabulary probe such as `wcf-service` maps to
    `dependency-surface terminal context`, not legacy-communication context.
  - [x] Assert the literal `dependency-surface` surface kind maps to
    `dependency-surface terminal context`.
  - [x] Assert remaining non-HTTP closed-vocabulary values map to
    `dependency-surface terminal context` unless a more specific bucket is
    documented.
  - [x] Assert `http-route` and `http-client` do not render property-flow
    terminal context under this slice even when an exact selected-property
    bridge is present. Direct mapper coverage asserts these surface kinds map
    to no terminal context; full HTTP bridge fixtures remain covered by the
    existing endpoint-proximity negative.
  - [x] Make the test fail closed when a new combined terminal surface kind is
    added without a mapped, suppressed, or deferred property-flow decision.
  - [x] Add representative full report fixtures for selected buckets; do not
...
- [ ] 2. Add exact selected-property bridge tests.
  ...
  - [ ] Isolate selected combined fact ID in at least one fixture when
    ...
    bridge.
  - [x] Cover selected symbol ID or selected target symbol as an exact bridge.
  ...
  - [x] Cover selected type-qualified DTO property as an exact bridge.
  - [ ] Cover selected type-qualified UI/model binding display where fixture
    data can represent it without scanner changes.
  - [x] Assert terminal context does not upgrade path classification beyond the
    weakest required evidence.
  - [x] Record any bridge family that cannot be represented in current tests
...
- [ ] 3. Add proximity-only negative tests.
  ...
  - [ ] Same short symbol or method name alone does not attach terminal
    context.
  - [x] Current property-flow generic selected names (`id`, `name`, `type`,
    `value`, `state`, `status`) do not attach terminal context without exact
    selected-property identity.
  - [x] Broader route-flow/high-fan-out generic examples such as `result` or
...
- [x] 4. Lock rule-catalog and report-version decisions.
  Requirements: 4, 5.
  - [x] Assert emitted path/surface rule IDs resolve in `rules/rule-catalog.yml`.
  - [x] Assert terminal-context notes and `terminalContextKind` metadata remain
    additive presentation over existing path/source rules, not a new rule-less
    conclusion.
  - [x] Reuse existing rules when terminal context remains additive note/safe
    metadata over existing combined path evidence.
  - [x] Add catalog entries, limitations, and tests before emitting any new
    terminal-context rule ID or gap code.
  - [x] Keep report version `1.0` only if metadata remains additive and safely
    ignorable.
  - [x] Add a report-version pin asserting `report.Version == "1.0"` when the
    chosen decision is additive metadata without a version bump.
  - [x] Bump report version if terminal context becomes required or changes
    existing path, node, edge, inventory, gap, summary, or Markdown semantics.
  - [x] Record the final rule/version decision in implementation state before
    product edits.
  - [x] Assert terminal-context notes and `terminalContextKind` metadata do not
...
- [x] 5. Validate PR 1.
  ...
  - [x] Update implementation state with validation evidence, deferred checks,
    and readiness.
